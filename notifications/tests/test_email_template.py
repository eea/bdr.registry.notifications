...
class CycleEmailTemplateTest(BaseTest):
    def setUp(self):
        ...
        self._EDIT_DATA = {"subject": "Subject Test", "body_html": "Body Test"}
    ...
    def test_cycle_email_template_view(self):
        ...
        cycle_email_template = factories.CycleEmailTemplateFactory(
            subject="Test view cycle email template", stage=stage
        )
        resp = self.client.get(
            reverse(
                "notifications:template:view", kwargs={"pk": cycle_email_template.pk}
            ),
            follow=True,
        )
    ...
    def test_cycle_edit(self):
        ...
        stage = factories.StageFactory(cycle=cycle)
        cycle_email_template = factories.CycleEmailTemplateFactory(
            subject="Test edit cycle", stage=stage
        )
        resp = self.client.post(
            reverse(
                "notifications:template:edit", kwargs={"pk": cycle_email_template.pk}
            ),
            self._EDIT_DATA,
            follow=True,
        )
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.context["object"].subject, self._EDIT_DATA["subject"])
        self.assertEqual(resp.context["object"].body_html, self._EDIT_DATA["body_html"])
    ...
    def test_cycle_email_test(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(
            resp.context["template"].body_html, self._EDIT_DATA["body_html"]
        )
    ...
    def test_cycle_email_test_objects(self):
        ...
        resp = self.client.post(
            reverse(
                "notifications:template:test", kwargs={"pk": self.cycle_template.pk}
            ),
            follow=True,
        )
    ...
    def test_cycle_email_trigger(self):
        ...
        resp = self.client.post(
            reverse(
                "notifications:template:trigger", kwargs={"pk": self.cycle_template.pk}
            ),
        ...
        for person in self.persons:
            body_expected = self.generate_body_for_person(
                {"CONTACT": person.name, "COMPANY": person.company.first().name}
            )
    ...
    def test_cycle_email_trigger_objects(self):
        ...
        self.assertEqual(self.cycle_template, resp.context["template"])
        self.assertEqual(
            len(self.persons), len(resp.context["recipients"].first().active_users)
        )
