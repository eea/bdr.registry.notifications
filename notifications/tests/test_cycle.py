...
class CycleTest(BaseTest):
    ...
    def test_cycle_add(self):
        resp = self.client.post(
            reverse("notifications:cycle:add"), self._DATA, follow=True
        )
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(len(resp.context["items"]), 1)
        self.assertEqual(resp.context["items"][0].year, self._DATA["year"])
        self.assertEqual(
            str(resp.context["items"][0].closing_date), self._DATA["closing_date"]
        )
    ...
    def test_cycle_view(self):
        ...
        resp = self.client.get(
            reverse("notifications:cycle:view", kwargs={"pk": cycle.pk}), follow=True
        )
        self.assertEqual(resp.status_code, 200)
