...
class CycleEmailTemplateBase(NotificationsBaseView):
    ...
    def breadcrumbs(self):
        ...
        breadcrumbs.extend(
            [
                ...
                Breadcrumb(
                    reverse(
                        "notifications:template:view", kwargs={"pk": self.object.pk}
                    ),
    ...
    def get_recipients(self, company_ids=None):
        qs = Person.objects.filter(company__group=self.object.group)
    ...
    def get_recipient_companies(self, company_ids=None):
        qs = (
            Company.objects.filter(group=self.object.group)
            .order_by("name")
            .prefetch_related("personcompany_set")
        )
        if company_ids is not None:
...
class CycleEmailTemplateEdit(CycleEmailTemplateBase, generic.UpdateView):
    ...
    def get_initial(self):
        ...
        except CycleEmailTemplate.DoesNotExist:
            return {}

    def get_context_data(self, **kwargs):
        context = super(CycleEmailTemplateEdit, self).get_context_data(**kwargs)
        last_2_cycles_ids = [
            cycle.id for cycle in Cycle.objects.order_by("-closing_date")[:2]
        ]
        old_templates = CycleEmailTemplate.objects.filter(
            status=CycleEmailTemplate.SENT, stage__cycle_id__in=last_2_cycles_ids
        ).order_by("group__title")
        context["old_templates"] = old_templates
        return context

    def get_object(self):
        obj = get_object_or_404(CycleEmailTemplate, id=self.kwargs["pk"])
        if obj.is_triggered:
    ...
    def get_success_url(self):
        return reverse("notifications:template:view", args=[self.object.pk])
...
class CycleEmailTemplateTriggerDetail(CycleEmailTemplateBase, generic.TemplateView):
    ...
    def get_company_ids(self):
        if "csv_file" not in self.request.FILES:
            return None
        reader = csv.reader(
            self.request.FILES["csv_file"].read().decode("utf-8-sig").splitlines()
        )
    ...
    def get_context_data(self, **kwargs):
        context = super(CycleEmailTemplateTriggerDetail, self).get_context_data(
            **kwargs
        )
        ...
        if company_ids is not None:
            found_companies = set(
                map(lambda x: x[0], companies.values_list("external_id"))
            )
    ...
    def get_notifications(self):
        return CycleNotification.objects.filter(emailtemplate=self.object).order_by(
            "-sent_date"
        )
...
class CycleEmailTemplateTriggerSend(generic.detail.SingleObjectMixin, generic.FormView):
    ...
    def get_success_url(self):
        return reverse("notifications:template:trigger", args=[self.kwargs["pk"]])

    def form_valid(self, form):
        companies = Company.objects.filter(
            id__in=json.loads(self.request.POST["recipients"])
        )
...
class CycleEmailTemplateTriggerNotificationsJson(JSONResponseMixin, TemplateView):
    ...
    def get_sort(self, order, direction):
        if self.object.group.code in BDR_GROUP_CODES:
            sorting = [
                "external_id",
                "name",
                "personcompany__person__name",
                "personcompany__person__email",
            ]
        else:
            sorting = [
                "integer_external_id",
    ...
    def get_companies_ecr(self, companies, order):
        return (
            companies.filter(personcompany__current=True)
            .values_list(
                "external_id",
                "name",
                "personcompany__person__name",
    ...
    def get_data(self, context):
        ...
        if self.object.is_triggered:
            companies = (
                Company.objects.filter(notifications__emailtemplate=self.object)
                .all()
                .prefetch_related("personcompany_set")
            )
        companies = self.get_recipient_companies(
            self.request.POST.getlist("filtered_companies[]", None)
        )
        if self.object.group.code in BDR_GROUP_CODES:
        ...
        if search_value:
            companies = (
                ...
                .filter(search__contains=search_value)
                .distinct()
            )
...
class CycleEmailTemplateTest(CycleEmailTemplateBase, generic.FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # TODO Create a function that takes param values, body_html and
    ...
    def get_success_url(self):
        ...
        return (
            reverse("notifications:template:test", args=[self.object.pk])
            + "?"
            + query_string
        )
...
class ViewSentNotificationForCompany(NotificationsBaseView, generic.DetailView):
    ...
    def get_object(self):
        return get_object_or_404(CompaniesGroup, id=self.kwargs["pk"])

    def get_company(self):
        return get_object_or_404(
            Company, id=self.kwargs["pk_company"], group=self.get_object()
        )
